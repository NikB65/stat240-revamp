...
Let's give the penguins dataset a rest and practice dplyr and grouping a bit more with a different dataset. The following chunk imports [`fertility.csv`](data/fertility.csv), the cleaned global [total fertility rate](https://en.wikipedia.org/wiki/Total_fertility_rate) (TFR) data set from [World Bank](https://data.worldbank.org/indicator/SP.DYN.TFRT.IN), giving the average TFR (number of children born per woman over their lifetime based on current conditions) for each year and country from 1960 to present. For the remainder of the notes, we will simply refer to TFR as "fertility". For the past few decades, global fertility has been [sharply declining](https://ourworldindata.org/un-population-2024-revision) for most countries. Many countries are now below the [replacement rate of 2.1](https://en.wikipedia.org/wiki/Sub-replacement_fertility), leading to widespread concerns of a population collapse in the latter part of the 21^st^ century.
